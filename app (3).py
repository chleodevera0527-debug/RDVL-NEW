...
def layout(title, body, user=None, refresh=False):
    if user:
        ...
        # Navigation is presentation-only. Existing URLs, permissions and handlers remain unchanged.
        links = [
            ("Dashboard", "/", "⌂", True),
            ("Trips", "/", "▣", False),
        ]
        if role in ("Admin", "Dispatcher"):
            links.append(("New Trip", "/new", "+", False))
        links += [("Map", "/map", "⌖", False), ("Calendar", "/analytics", "□", False)]
        if role == "Admin":
        ...
        for label, href, icon, active in links:
            cls = "nav-link active" if active and title == "Dashboard" else "nav-link"
            nav_items.append(f'<a class="{cls}" href="{href}"><span class="nav-icon">{icon}</span><span>{label}</span></a>')
    ...
    mobile_menu = '<button class="mobile-menu" type="button" onclick="document.body.classList.toggle(\'mobile-nav-open\')">☰</button>'
...
<style>
:root{
 --bg:#f7f9fc;--surface:#fff;--surface-soft:#f8fafc;--border:#e2e8f0;--border-2:#d7e0ec;
 --text:#14213a;--muted:#64748b;--muted-2:#94a3b8;--primary:#2563eb;--primary-dark:#1d4ed8;
 --success:#16a34a;--warning:#f59e0b;--danger:#ef4444;--shadow:0 8px 28px rgba(15,23,42,.055);
 --shadow-lg:0 18px 50px rgba(15,23,42,.10);--radius:12px;--radius-sm:8px
}
*{box-sizing:border-box}html{background:var(--bg)}body{margin:0;min-height:100vh;color:var(--text);background:linear-gradient(180deg,#fff 0,#f7f9fc 230px);font:14px Inter,ui-sans-serif,system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;-webkit-font-smoothing:antialiased}
a{color:inherit;text-decoration:none}button,input,select,textarea{font:inherit}
.topbar{position:sticky;top:0;z-index:100;background:rgba(255,255,255,.96);backdrop-filter:blur(14px);border-bottom:1px solid var(--border);height:84px;display:flex;align-items:center;padding:0 30px;gap:28px;box-shadow:0 1px 4px rgba(15,23,42,.035)}
.brand{display:flex;align-items:center;gap:10px;min-width:194px;flex-shrink:0}.brand-mark{width:42px;height:42px;border-radius:10px;background:#eff6ff;color:var(--primary);display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:900}.brand-copy{line-height:1}.brand-main{font-size:18px;font-weight:900;letter-spacing:-.55px}.brand-sub{font-size:10px;color:var(--primary);font-weight:850;letter-spacing:.45px;margin-top:4px}
.nav{display:flex;align-items:center;gap:4px;min-width:0}.nav-link{height:44px;display:inline-flex;align-items:center;gap:8px;padding:0 13px;border-radius:9px;color:#334155;font-size:14px;font-weight:650;white-space:nowrap;transition:.16s}.nav-link:hover{background:#f1f5f9;color:#0f172a}.nav-link.active{background:#eef4ff;color:#1d4ed8;font-weight:800}.nav-icon{font-size:18px;line-height:1}
.top-spacer{flex:1}.top-meta{display:flex;align-items:center;gap:18px}.top-meta-block{display:flex;align-items:center;gap:8px;color:var(--muted);font-size:12px}.top-icon{font-size:18px;color:var(--primary)}.top-date{white-space:nowrap}.weather-head{display:flex;align-items:center;gap:8px}.weather-head .sun{font-size:22px}.weather-head strong{font-size:12px;display:block;color:#1e293b}.weather-head small{font-size:10px;color:var(--muted)}
.account-menu{position:relative}.account-menu summary{list-style:none;display:flex;align-items:center;gap:9px;padding:4px;cursor:pointer}.account-menu summary::-webkit-details-marker{display:none}.account-avatar{width:34px;height:34px;border-radius:50%;background:var(--primary);color:#fff;display:inline-flex;align-items:center;justify-content:center;font-size:12px;font-weight:850;flex-shrink:0}.account-avatar.large{width:42px;height:42px}.account-summary{display:flex;flex-direction:column;line-height:1.2}.account-summary b{font-size:13px}.account-summary small{font-size:10px;color:var(--muted);margin-top:2px}.account-chevron{font-size:15px;color:#64748b}.account-dropdown{position:absolute;right:0;top:46px;width:275px;background:#fff;border:1px solid var(--border);border-radius:12px;box-shadow:var(--shadow-lg);padding:12px;z-index:500}.account-card-head{display:flex;align-items:center;gap:10px;padding:3px 2px 11px;border-bottom:1px solid var(--border)}.account-card-head div{display:flex;flex-direction:column}.account-card-head strong{font-size:13px}.account-card-head span{font-size:10px;color:var(--muted);margin-top:2px}.account-meta{display:grid;grid-template-columns:65px 1fr;gap:5px 8px;padding:11px 2px;font-size:11px}.account-meta span{font-size:9px;color:var(--muted-2);font-weight:850}.account-link,.account-logout{display:block;padding:9px;border-radius:8px;font-size:12px;font-weight:700}.account-link:hover,.account-logout:hover{background:#f4f7fb}.account-logout{border:0;background:transparent;width:100%;text-align:left}.linkbtn{border:0!important;background:transparent!important;box-shadow:none!important;color:#dc2626!important;padding:0!important}
.mobile-menu{display:none;width:38px;height:38px;border:1px solid var(--border);border-radius:8px;background:#fff;color:#1e293b;align-items:center;justify-content:center;cursor:pointer}
.page{min-height:calc(100vh - 84px)}.content{max-width:1640px;margin:0 auto;padding:28px 28px 55px}.page-title{font-size:25px;font-weight:850;margin:0 0 5px;letter-spacing:-.55px}.page-subtitle{font-size:13px;color:var(--muted);margin-bottom:20px}.card{background:var(--surface);border:1px solid var(--border);border-radius:13px;box-shadow:var(--shadow);margin-bottom:18px}.notice,.ok,.error,.issue{padding:12px 14px;border-radius:10px;margin-bottom:14px;border:1px solid}.notice,.ok{background:#ecfdf3;color:#166534;border-color:#bbf7d0}.error,.issue{background:#fef2f2;color:#991b1b;border-color:#fecaca}
.live{display:inline-flex;align-items:center;gap:7px;background:#ecfdf5;color:#087443;border:1px solid #c9f0dc;border-radius:999px;padding:6px 10px;font-size:10px;font-weight:750;margin-bottom:12px}.live:before{content:"";width:6px;height:6px;border-radius:50%;background:#16a34a}
.search-card{padding:18px 20px 20px;overflow:hidden;background:#fff}.search-row{display:flex;align-items:end;gap:10px}.search-label{font-weight:800;font-size:13px;color:#1e293b;display:block;margin-bottom:7px}.search-box{height:44px;flex:1;position:relative}.search-box input{width:100%;height:44px;margin:0;padding:0 42px 0 14px;border:1px solid var(--border-2);border-radius:9px;background:#fff;outline:none;color:#24324a}.search-box input::placeholder{color:#94a3b8}.search-box input:focus{border-color:#7aa8f8;box-shadow:0 0 0 3px rgba(37,99,235,.08)}.search-box:after{content:"⌕";position:absolute;right:14px;top:7px;font-size:23px;color:#64748b}.filter-btn,.clear-btn,.new-btn{height:44px;padding:0 17px;border-radius:9px;font-weight:800;border:1px solid var(--border-2);background:#fff;color:var(--primary);cursor:pointer;display:inline-flex;align-items:center;justify-content:center;gap:7px}.filter-btn{background:var(--primary);border-color:var(--primary);color:#fff}.filter-btn:hover{background:var(--primary-dark)}.clear-btn:hover{background:#f8fafc}.new-btn{background:var(--primary);color:#fff;border-color:var(--primary)}
.status-row{display:grid;grid-template-columns:repeat(10,minmax(0,1fr));gap:10px;margin-top:16px}.status-filter{min-width:0;height:68px;border:1px solid var(--border);border-radius:9px;background:#fff;display:flex;flex-direction:column;justify-content:center;padding:8px 12px;transition:.16s}.status-filter:hover{border-color:#b8c9e2;box-shadow:0 3px 12px rgba(15,23,42,.05)}.status-filter.active{border-color:#bfd3fb;box-shadow:0 0 0 1px #e7efff inset}.status-filter>span:first-child{font-size:11px;color:#64748b;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.count-pill{font-size:20px;font-weight:850;color:var(--primary);margin-top:3px;line-height:1}
.table-card{overflow:hidden}.table-head{min-height:70px;display:flex;align-items:center;justify-content:space-between;padding:0 18px;border-bottom:1px solid var(--border)}.table-title{font-size:18px;font-weight:850;display:flex;align-items:center;gap:9px}.table-title-icon{font-size:19px;color:var(--primary)}.columns-btn{height:38px;padding:0 13px;border:1px solid var(--border-2);border-radius:8px;background:#fff;color:#1d4ed8;font-weight:750}.table-wrap{width:100%;overflow:auto}.dashboard-table{width:100%;min-width:1220px;border-collapse:separate;border-spacing:0}.dashboard-table th{height:47px;background:#f8fafc;color:#51627a;border-top:1px solid var(--border);border-bottom:1px solid var(--border);padding:0 12px;text-align:left;font-size:10px;text-transform:uppercase;letter-spacing:.045em;white-space:nowrap}.dashboard-table td{height:66px;padding:10px 12px;border-bottom:1px solid #edf1f5;vertical-align:middle;color:#334155;background:#fff}.dashboard-table tbody tr:hover td{background:#fbfdff}.container-link{display:block;color:#1e3a8a;font-weight:850}.cargo-chip{display:inline-block;margin-top:4px;font-size:10px;color:#64748b;background:#f1f5f9;border-radius:999px;padding:2px 7px}.driver-cell strong{display:block;color:#1e293b;font-size:12px}.driver-phone{display:block;color:#94a3b8;font-size:10px;margin-top:3px}.shipping-cell{white-space:nowrap}.shipping-dot{color:#2563eb;margin-right:5px}.status-chip,.priority-chip{display:inline-flex;align-items:center;border-radius:999px;padding:5px 9px;font-size:10px;font-weight:800;white-space:nowrap}.status-dispatched{background:#eff6ff;color:#1d4ed8}.status-port{background:#ecfeff;color:#0f766e}.status-transit{background:#eff6ff;color:#2563eb}.status-delivery{background:#fff7ed;color:#c2410c}.status-unloading{background:#f5f3ff;color:#6d28d9}.status-yard{background:#f0fdfa;color:#0f766e}.status-returned{background:#fef2f2;color:#dc2626}.status-completed{background:#ecfdf3;color:#15803d}.status-hold{background:#fff7ed;color:#b45309}.priority-chip{background:#f8fafc;color:#64748b;border:1px solid #e5e7eb}.location-cell{max-width:180px}.milestone-cell strong{display:block;font-size:11px;color:#334155}.milestone-cell small{display:block;font-size:10px;color:#94a3b8;margin-top:3px}.actions-cell{white-space:nowrap}.action-buttons{display:flex;align-items:center;gap:6px}.action-btn{height:32px;display:inline-flex;align-items:center;padding:0 9px;border:1px solid #dbe3ee;border-radius:7px;background:#fff;color:#1d4ed8;font-size:10px;font-weight:800}.action-btn:hover{background:#eff6ff}.action-menu{position:relative}.action-menu summary{list-style:none;width:31px;height:32px;border:1px solid #dbe3ee;border-radius:7px;display:flex;align-items:center;justify-content:center;cursor:pointer;color:#64748b}.action-menu summary::-webkit-details-marker{display:none}.action-menu-items{position:absolute;right:0;top:37px;width:145px;background:#fff;border:1px solid var(--border);border-radius:8px;box-shadow:var(--shadow-lg);padding:5px;z-index:20}.action-menu-items a{display:block;padding:8px 9px;border-radius:6px;font-size:11px}.action-menu-items a:hover{background:#f1f5f9}.pagination{min-height:64px;display:flex;align-items:center;justify-content:space-between;padding:0 18px;color:#64748b;font-size:11px}.page-controls{display:flex;align-items:center;gap:6px}.page-btn,.page-number,.per-page{height:36px;border:1px solid var(--border-2);background:#fff;border-radius:8px;color:#64748b;font-weight:750}.page-btn{padding:0 10px}.page-number{width:36px;background:var(--primary);border-color:var(--primary);color:#fff}.per-page{padding:0 9px;margin-left:4px}.empty{padding:72px 20px!important;text-align:center!important;color:#64748b!important;font-size:12px!important}.empty:before{content:"⌑";display:block;font-size:38px;color:#cbd5e1;margin-bottom:8px}
.statgrid{display:none}.toolbar{display:flex;gap:10px;align-items:end}.toolbar label{font-weight:700;color:#334155;flex:1}.toolbar input,.toolbar select,.toolbar textarea{width:100%;margin-top:6px}.toolbar button,.button{height:42px;padding:0 15px;border-radius:9px;border:0;background:var(--primary);color:#fff;font-weight:800}.button.secondary{background:#eef2f7;color:#334155}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(220px,1fr));gap:14px}input,select,textarea{border:1px solid var(--border-2);border-radius:9px;padding:10px 12px;background:#fff;color:#172033;outline:none}input:focus,select:focus,textarea:focus{border-color:#76a8fb;box-shadow:0 0 0 3px rgba(37,99,235,.08)}label{display:block;font-weight:650;color:#334155}textarea{min-height:90px;resize:vertical}.wide{grid-column:1/-1}h2{font-size:24px;letter-spacing:-.6px;margin:0 0 18px}h3{font-size:16px;margin:0 0 14px}.muted{color:#718096;font-size:11px}.small{font-size:10px;color:#94a3b8}.table-card table{width:100%;border-collapse:collapse}.table-card table th{background:#f8fafc}.table-card table th,.table-card table td{padding:11px 10px;border-bottom:1px solid #edf1f5;text-align:left;vertical-align:top}
@media(max-width:1280px){.nav-link{padding:0 9px;font-size:12px}.nav-icon{font-size:16px}.topbar{gap:14px;padding:0 20px}.brand{min-width:175px}.status-row{grid-template-columns:repeat(5,minmax(0,1fr))}.dashboard-table{min-width:1180px}}
@media(max-width:1000px){.nav{overflow:auto}.nav-link span:last-child{display:none}.nav-link{width:38px;justify-content:center;padding:0}.top-date,.weather-block{display:none}.status-row{grid-template-columns:repeat(5,minmax(0,1fr))}.content{padding:22px 18px 45px}}
@media(max-width:760px){.topbar{height:72px;padding:0 12px}.mobile-menu{display:inline-flex}.brand{min-width:auto}.brand-copy{display:none}.nav{position:absolute;left:8px;right:8px;top:72px;background:#fff;border:1px solid var(--border);border-radius:0 0 12px 12px;box-shadow:var(--shadow-lg);padding:8px;display:none;flex-wrap:wrap}.mobile-nav-open .nav{display:flex}.nav-link{width:auto;justify-content:flex-start;padding:0 10px;flex:1 1 120px}.nav-link span:last-child{display:inline}.page{min-height:calc(100vh - 72px)}.content{padding:18px 12px 35px}.page-title{font-size:22px}.search-card{padding:14px}.search-row{flex-wrap:wrap;align-items:stretch}.search-label{width:100%;margin-bottom:-2px}.search-box{width:100%;flex-basis:100%}.filter-btn,.clear-btn,.new-btn{height:40px}.status-row{grid-template-columns:repeat(2,minmax(0,1fr));gap:8px;margin-top:12px}.status-filter{height:58px}.account-summary{display:none}.table-head{min-height:60px}.pagination{flex-wrap:wrap;gap:10px;padding:12px}.page-controls{margin-left:auto}}
@media(max-width:430px){.topbar{gap:8px}.account-chevron{display:none}.status-row{grid-template-columns:1fr 1fr}.filter-btn,.clear-btn{flex:1}.dashboard-table{min-width:1100px}}
</style></head><body>'''

    if user:
        html += f'''<header class="topbar">{mobile_menu}<a class="brand" href="/"><span class="brand-mark">▣</span><span class="brand-copy"><span class="brand-main">R. DEVERA</span><span class="brand-sub">LOGISTICS SERVICES</span></span></a><nav class="nav">{nav_html}</nav><div class="top-spacer"></div><div class="top-meta"><div class="top-meta-block"><span class="top-icon">◷</span><span id="live-date" class="top-date">Loading...</span></div><div class="top-meta-block weather-block"><div class="weather-head"><span class="sun">☀</span><div><strong id="top-temp">29°C</strong><small id="top-location">Manila, PH</small></div></div></div>{account_html}</div></header>'''
    else:
        html += '<header class="topbar"><a class="brand" href="/login"><span class="brand-mark">▣</span><span class="brand-copy"><span class="brand-main">R. DEVERA</span><span class="brand-sub">LOGISTICS SERVICES</span></span></a></header>'

    html += '<div class="page"><main class="content">' + body + '</main></div>' + refresh_script + '''
...
def dashboard(query, user, notice=""):
    ...
    status_items = [
        ("All trips", len(rows), ""),
        ("Dispatched", counts.get("Dispatched", 0), "Dispatched"),
        ("At Port", counts.get("At Port", 0), "At Port"),
        ("In Transit", counts.get("In Transit", 0), "In Transit"),
        ("At Delivery Site", counts.get("At Delivery Site", 0), "At Delivery Site"),
        ("Unloading", counts.get("Unloading", 0), "Unloading"),
        ("In Court Yard", counts.get("In Court Yard", 0), "In Court Yard"),
        ("Returned to Port", counts.get("Returned to Port", 0), "Returned to Port"),
        ("Completed", counts.get("Completed", 0), "Completed"),
        ("On Hold", counts.get("On Hold", 0), "On Hold"),
    ]
    status_html=[]
    ...
    for d in filtered:
        ...
        contact=phone
        court_yard=d.get("court_yard_name") or "—"
        location=d.get("current_location") or court_yard or "—"
        ...
        # Keep the existing Viewer permission boundary: Viewer rows omit restricted contact/notes fields.
        contact_cell = '<td>—</td>' if is_viewer else f'<td>{escape(contact)}</td>'
        notes_cell = '<td>—</td>' if is_viewer else f'<td class="notes-cell">{escape(notes)}</td>'
        rows_html.append(f'''<tr data-trip-id="{d['id']}">
<td><a class="container-link" href="/trip/{d['id']}">{escape(d['container_no'])}</a><span class="cargo-chip">{escape(client)}</span></td>
<td class="driver-cell"><strong>{escape(driver)}</strong><span class="driver-phone">{escape(phone)}</span></td>
<td>{escape(trucker)}</td>
...
<td><span class="cargo-chip">{escape(cargo)}</span></td>
<td>{escape(court_yard)}</td>
...
<div style="display:flex;align-items:flex-end;justify-content:space-between;gap:14px;flex-wrap:wrap;margin-bottom:18px">
  <div><h1 class="page-title">Dashboard 👋</h1><div class="page-subtitle">Real-time overview of all trips and their status</div></div>
  <div class="live" style="margin:0">Live database view · refreshes every {LIVE_REFRESH_SECONDS} seconds</div>
</div>
<div class="card search-card">
  <form method="get">
    <label class="search-label">Search</label>
    <div class="search-row">
      <div class="search-box"><input name="q" value="{escape(q, quote=True)}" placeholder="Container, driver, trucker, shipping line, status..."></div>
...
<div class="card table-card">
  <div class="table-head"><div class="table-title">All Trips <span class="muted">{len(filtered)} total</span></div><a class="columns-btn" href="/export">⇩&nbsp; Export</a></div>
  <div class="table-wrap"><table class="dashboard-table">
    <thead><tr><th>Container / Client</th><th>Driver</th><th>Trucker</th><th>Shipping Line</th><th>Contact</th><th>Status</th><th>Cargo</th><th>Court Yard</th><th>Location</th><th>Notes</th><th>Latest Update</th><th>Actions</th></tr></thead>
    <tbody>{''.join(rows_html) or f'<tr><td colspan="{col_count}" class="empty">No trips found.<br><span class="muted">Try adjusting your search or filters.</span></td></tr>'}</tbody>
  </table></div>
  <div class="pagination"><span>Showing 0 to {len(filtered)} of {len(filtered)} entries</span><div class="page-controls"><button class="page-btn" disabled>First</button><button class="page-btn" disabled>Previous</button><button class="page-number">1</button><button class="page-btn" disabled>Next</button><button class="page-btn" disabled>Last</button><select class="per-page"><option>25</option><option>50</option><option>100</option></select></div></div>
</div>
'''
