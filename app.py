...
STATUSES = [
    "Dispatched", "At Port", "In Transit", "At Delivery Site",
    "Unloading", "In Court Yard", "Returned to Port",
    "Completed", "On Hold"
]
...
def install_database_guards(con):
    ...
    deps = [
        ...
        ("returned_port", "yard_pullout", "Returned to port requires a prior yard pullout event."),
    ]
...
def _initialize_database():
    """Create/migrate the database exactly once per process."""
        ...
        if admin_password:
            ...
            if not existing_admin:
                con.execute(
                    ...
                    (admin_user, hash_password(admin_password), "Admin", now_iso())
                )
            elif os.getenv("ADMIN_PASSWORD", "").strip():
                                                                                                    
                con.execute(
                    "UPDATE users SET password_hash=?, role='Admin', active=1 WHERE username=?",
                    (hash_password(admin_password), admin_user)
                )
        con.commit()
...
def master_choices(table):
    with closing(db()) as con:
        rows = con.execute(f"SELECT name FROM {table} WHERE active=1 ORDER BY name COLLATE NOCASE").fetchall()
...
(function(){{function c(){{const n=new Date(),d=document.getElementById("live-date"),t=document.getElementById("live-time");if(d)d.textContent=n.toLocaleDateString(undefined,{{weekday:"short",month:"short",day:"numeric",year:"numeric"}});if(t)t.textContent=n.toLocaleTimeString(undefined,{{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:true}});}}c();setInterval(c,1000);}})();
</script>
</body></html>'''

def app_secret():
    """Return a stable cryptographic secret without falling back to a public constant."""
    if APP_SECRET:
...
function clock(){{const n=new Date(),d=document.getElementById("dashboard-date"),t=document.getElementById("dashboard-time");if(d)d.textContent=n.toLocaleDateString(undefined,{{weekday:"long",year:"numeric",month:"long",day:"numeric"}});if(t)t.textContent=n.toLocaleTimeString(undefined,{{hour:"2-digit",minute:"2-digit",second:"2-digit",hour12:true}});}}
...
def audit_page(user):
    ...
    issues=[]; seen={}
    if integrity != "ok":
        issues.append("SQLite integrity check failed: " + str(integrity))
    if foreign_keys:
    ...
    for d in rows:
        ...
        if d.get("returned_port") and not d.get("yard_pullout"):
            issues.append(f"{c}: returned to port is recorded without yard pullout.")
...
def master_data_page(user, notice=""):
    if user["role"]!="Admin":
        return layout("Forbidden",'<div class="error">Admin access required.</div>',user)
    with closing(db()) as con:
        truckers=con.execute("SELECT id,name,active,created_at FROM truckers ORDER BY name COLLATE NOCASE").fetchall()
        clients=con.execute("SELECT id,name,active,created_at FROM clients ORDER BY name COLLATE NOCASE").fetchall()
...
def validate_milestone_update(old, new):
    """Milestone timestamps are append-only business events once recorded."""
    errors = []
    for key, label in TIMES:
        before = old.get(key)
        after = new.get(key)
...
def export_page(query,user):
    ...
    with closing(db()) as con:
        rows=con.execute(f"SELECT * FROM trips{where} ORDER BY port_arrival,id",params).fetchall(); cols=[r["name"] for r in con.execute("PRAGMA table_info(trips)")]
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if p.path=="/health": return self.send_html(json.dumps({"ok":True,"database":str(DB.name)}),200,[("Content-Type","application/json")])
    ...
    def do_POST(self):
        ...
        if p.path=="/users":
            ...
            if action=="edit":
                ...
                with closing(db()) as con:
                    ...
                    try:
                        ...
                        con.commit()
                    except sqlite3.IntegrityError: return self.send_html(user_edit_page(user,ident,"Username already exists."),400)
                return self.redirect("/users")
            ...
            with closing(db()) as con:
                ...
                except sqlite3.IntegrityError as e: return self.send_html(users_page(user)+f'<div class=error>{escape(str(e))}</div>',400)
            return self.redirect("/users")
        ...
        if p.path=="/master-data":
            ...
            if action=="edit":
                ...
                with closing(db()) as con:
                    ...
                    try:
                        ...
                        con.commit()
                    except sqlite3.IntegrityError: return self.send_html(master_data_edit_page(user,kind,ident,"That name already exists."),400)
                return self.redirect("/master-data")
            ...
            with closing(db()) as con:
                ...
                except sqlite3.IntegrityError: return self.send_html(master_data_page(user,"That name already exists."),400)
            return self.redirect("/master-data")
...
def app(environ,start_response):
    ...
    initialize()
    path=environ.get("PATH_INFO","/"); query=environ.get("QUERY_STRING",""); method=environ.get("REQUEST_METHOD","GET")
    headers=dict(environ.get("HTTP_COOKIE","") and [("Cookie",environ.get("HTTP_COOKIE"))] or [])
    cookie=environ.get("HTTP_COOKIE",""); token=""
    for part in cookie.split(";"):
        if part.strip().startswith("session="): token=part.strip().split("=",1)[1]
    with closing(db()) as con: user=current_user(con,token)

    if path=="/health":
        body=json.dumps({"ok":True,"database":DB.name}).encode(); start_response("200 OK",[("Content-Type","application/json"),("Content-Length",str(len(body)))]+security_wsgi()); return [body]
    if not user and path not in ("/login",):
        body=login_page().encode(); start_response("401 Unauthorized",[("Content-Type","text/html; charset=utf-8"),("Content-Length",str(len(body)))]+security_wsgi()); return [body]
    if path=="/login" and method=="GET":
        body=login_page().encode(); start_response("200 OK",[("Content-Type","text/html; charset=utf-8"),("Content-Length",str(len(body)))]+security_wsgi()); return [body]
    if path=="/" and method=="GET": content=dashboard(parse_qs(query),user)
    elif path=="/analytics" and method=="GET": content=analytics_page(user)
    elif path=="/alerts" and method=="GET": content=alerts_page(user)
    elif path=="/activity" and method=="GET": content=activity_page(user)
    elif path=="/audit" and method=="GET": content=audit_page(user)
    elif path=="/map" and method=="GET": content=map_page(user)
    elif path=="/users" and method=="GET": content=users_page(user)
    elif path=="/new" and method=="GET": content=trip_form(None,user)
    else: content=layout("Unsupported",'<div class="error">Use the built-in HTTP server for this deployment unless your reverse proxy sends requests to the WSGI application through a framework adapter.</div>',user)
    body=content.encode(); start_response("200 OK",[("Content-Type","text/html; charset=utf-8"),("Content-Length",str(len(body)))]+security_wsgi()); return [body]
...
if __name__=="__main__":
    ...
    if os.getenv("ADMIN_PASSWORD"):
        print("Configured admin credentials loaded from environment.")
    elif os.getenv("ALLOW_DEFAULT_ADMIN", "0") == "1":
        print("WARNING: development default admin enabled; do not expose this server publicly.")
